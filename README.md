...
在项目开发初期，无论采用客户端或服务器端用法，都要想办法将用到的 css 或 less 文件引入到 html 页面或桥接文件中，less 中使用 import。可以通过 import 引入需要的 .less 或 .css 文件。 如：
``` js
...
less 中的变量和其他编程语言一样，可以实现值的复用，同样它也有生命周期，也就是 scope（变量范围，作用域），即局部变量 or 全局变量的概念，查找变量的顺序是先在局部定义中找，如果找不到，则查找上级定义，直至全局。
``` less
@width : 20px; 
#homeDiv { 
    @width : 30px; 
...
```
2. #### Mixin(混入)
同 Sass 一样，很多动态语言都支持 mixin（混入）特性，它是多重继承的一种实现。在 less 中，混入是指在一个 class 中引入另外一个已经定义的 class，就像在当前 class 中增加一个属性一样。
...
```
从上面的代码我们可以看出：mixin 其实是一种嵌套，它允许将一个类嵌入到另外一个类中使用，被嵌入的类也可以称作变量。
mixin 还有一种形式叫做 Parametric Mixins（混入参数），less 也支持这一特性：
``` less
// 定义一个样式选择器
 .borderRadius(@radius){ 
     -moz-border-radius: @radius; 
...
```
像 js 中 arguments 一样，mixin 也有 @arguments 变量。当 mixin 引用这个参数时，该参数表示所有的变量。很多情况下该参数可以省去很多代码。
``` less
...
```
mixin 是 less 中重要特性之一。在看了这么多例子之后，不免会存在这样的疑问：团队开发中当使用大量的选择器时如何解决选择器之间重名问题？在java 或 C++ 中使用了命名空间 namespaces，less 也同样采用了命名空间的方法避免重名问题，于是 less 在 mixin 的基础上扩展了一下，看下面这段代码：
``` less
#mynamespace { 
    .home {...} 
    .user {...} 
...
```
less 可以对数值型的 value（数字、颜色、变量等）进行加减乘除四则运算。同时 less 针对 color 操作提供了一组函数：
``` less
...
```
PS: 上述代码引自 less css 官方网站，详情请参见 [http://lesscss.org/#-color-functions]( http://lesscss.org/#-color-functions)
